...
public class EditProductController implements Initializable {
        ...
        if (!stockSize.getText().trim().isEmpty() && !stockLimit.getText().trim().isEmpty()
            ...
            if (Integer.valueOf(stockSize.getText().trim()) > Integer.valueOf(stockLimit.getText().trim()) && dateValid) {
                ...
                try {
                    ...
                    PreparedStatement prep = con.prepareStatement("{call sp_reflect_edit_update(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)}");
                    ...
                    prep.setInt(12, InventoryController.pc.getProdId());
                    ...
                    if (i > 0) {
                        Alert alert = new Alert(Alert.AlertType.INFORMATION);
                        alert.setTitle("Notification");
                        alert.setHeaderText("Successfully Edited " + prodName.getText() + "!");
    ...
    public void reflectEdit() {
        prodName.setText(pc.getProdName());
        prodPrice.setText(String.valueOf(pc.getProdPrice()));
        prodDesc.setText(pc.getProdDesc());
        catName.setText(pc.getCatName());
        catDesc.setText(pc.getCatDesc());
        stockSize.setText(String.valueOf(pc.getQuantity()));
        stockLimit.setText(String.valueOf(pc.getLimit()));
        manufactured.setPromptText(pc.getManufactured());
        expiration.setPromptText(pc.getExpiration());
        supplier.setText(pc.getSupplier());
    }
